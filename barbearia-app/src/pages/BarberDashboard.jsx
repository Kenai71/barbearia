...
import { format, isToday, startOfMonth, endOfMonth, eachDayOfInterval, addMonths, subMonths, getDay, isSameDay, startOfDay, isAfter, subDays, addMinutes } from 'date-fns';
...
export default function BarberDashboard({ session, isAdmin }) {
  ...
  const subscribeUser = async () => {
    ...
    if (VAPID_PUBLIC_KEY === 'VAPID_PUBLIC_KEY_PLACEHOLDER') {
        alert('A chave VAPID pública não foi configurada.');
        return;
    }
    ...
    try {
        ...
        alert('Falha ao se inscrever: ' + error.message);
    } finally {
              ...
              supabase
                  ...
                  .then(({ data }) => {
                      ...
                      setNewAppointmentNotification({
                          ...
                          day: format(appointmentDate, "dd 'de' MMMM", { locale: ptBR })
                      });
  ...
  const fetchAppointments = async () => {
    ...
    if (!error && data) {
      ...
      // NOVA LÓGICA DE FATURAMENTO: Considera apenas registros com preço preenchido
  ...
  // NOVA LÓGICA: Agenda Geral filtra cancelados com mais de 1 dia
  const activeAppointments = appointments.filter(app => {
    if (app.status === 'completed') return false;
    if (app.status === 'cancelled') {
        const oneDayAgo = subDays(new Date(), 1);
        return isAfter(new Date(app.date_time), oneDayAgo);
    }
    return true;
  });
      ...
      {newAppointmentNotification && (
        <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-right-10 duration-500">
          <div className="bg-green-600 text-white p-4 rounded-xl shadow-2xl flex items-center gap-3 max-w-sm">
            ...
            <button onClick={() => setNewAppointmentNotification(null)} className="ml-2 text-white/80">
              <X size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 border dark:border-slate-700">
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <input type="password" placeholder="Nova Senha" className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl dark:text-white outline-none focus:border-blue-500" value={newPassword} onChange={e => setNewPassword(e.target.value)} required />
              <input type="password" placeholder="Confirmar Senha" className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl dark:text-white outline-none focus:border-blue-500" value={confirmNewPassword} onChange={e => setConfirmNewPassword(e.target.value)} required />
            ...
            <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-7 gap-2 mb-6">
                ...
                {daysInMonth.map((date) => {
                  ...
                  return (
                    <button key={date.toString()} disabled={!config.active} onClick={() => setSelectedCalendarDate(date)} className={`aspect-square rounded-xl flex flex-col items-center justify-center text-sm font-bold transition-all relative ${isSelected ? 'bg-blue-600 text-white shadow-md' : ''} ${!isSelected && config.active ? 'bg-slate-50 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-blue-100' : ''} ${!config.active ? 'opacity-30 cursor-not-allowed' : ''}`}>
                      {format(date, 'd')}
                      {hasAppointments && !isSelected && config.active && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mt-1"></div>}
                    </button>
                  );
                })}
              </div>
              {selectedCalendarDate && (
                <div className="border-t border-slate-100 dark:border-slate-700 pt-4">
                  <h4 className="font-bold text-slate-900 dark:text-white mb-3 text-sm">Cortes em {format(selectedCalendarDate, "dd/MM")}</h4>
                  {selectedDayAppointments.length === 0 ? <p className="text-slate-400 text-xs text-center py-4">Vazio.</p> : (
                      ...
                      {selectedDayAppointments.map(app => (
                        <div key={app.id} className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 flex items-center gap-1"><Clock size={10} /> {format(new Date(app.date_time), 'HH:mm')}</p>
                          </div>
                          <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${app.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{app.status}</span>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      {showNoShowModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center border dark:border-slate-700">
            <div className="w-14 h-14 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300 rounded-full flex items-center justify-center mx-auto mb-4"><UserX size={28} /></div>
      ...
      <header className="bg-slate-900 dark:bg-black text-white pb-24 pt-8 px-6 shadow-xl transition-colors">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">{isAdmin ? 'Modo Administrador & Profissional' : 'Visão geral da equipe.'}</p>
          </div>
          <div className="flex items-center gap-4">
            <button onClick={isPushSubscribed ? unsubscribeUser : subscribeUser} disabled={loadingPass} className={`p-2 rounded-lg transition-all border shadow-lg flex items-center justify-center gap-2 ${isPushSubscribed ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/10 border-white/5 text-white'}`}>
              {loadingPass ? <Loader2 size={18} className="animate-spin" /> : <Bell size={18} />}
              <span className="hidden sm:inline">{isPushSubscribed ? 'Notificações Ativas' : 'Ativar Alertas'}</span>
            </button>
            {isAdmin && (
              <>
                <Link to="/admin/register-barber" className="p-2 rounded-lg bg-green-600 hover:bg-green-700 text-white border border-green-500 shadow-lg transition-all"><UserPlus size={18} /></Link>
                <Link to="/admin/settings" className="p-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white border border-blue-500 shadow-lg transition-all"><Settings size={18} /></Link>
              </>
            )}
            <button onClick={() => setShowPasswordModal(true)} className="p-2 rounded-lg bg-white/10 hover:bg-white/20 text-white border border-white/5 transition-all"><Lock size={18} /></button>
            <button onClick={() => supabase.auth.signOut()} className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg transition-colors text-sm font-medium border border-white/5"><LogOut size={16} /> Sair</button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-16 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between">
            ...
            <button onClick={() => setShowHistoryModal(true)} className="w-12 h-12 bg-green-50 dark:bg-green-900/30 rounded-xl flex items-center justify-center text-green-600 dark:text-green-400 hover:bg-green-100 hover:scale-105 transition-all cursor-pointer shadow-sm"><Wallet size={24} /></button>
          </div>
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-between">
            ...
            <button onClick={() => setShowCalendarModal(true)} className="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-xl flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-slate-200 hover:scale-105 transition-all cursor-pointer shadow-sm"><Clock size={24} /></button>
          </div>
        </div>

        {/* AGENDA GERAL COM FILTROS NOVOS */}
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {activeAppointments.length === 0 && <div className="p-12 text-center text-slate-500">Nenhum agendamento ativo.</div>}
            {activeAppointments.map((app) => {
              ...
              // EXIBIÇÃO DE INTERVALO DE 30 MINUTOS
              const timeRange = `${format(startTime, 'HH:mm')} às ${format(addMinutes(startTime, 30), 'HH:mm')}`;
                ...
                <div key={app.id} className={`p-6 transition-all flex flex-col sm:flex-row justify-between sm:items-start gap-4 group border-l-4 ${isMine ? 'bg-blue-50/40 dark:bg-blue-900/10 border-l-blue-600' : 'bg-white dark:bg-slate-800 border-l-transparent opacity-80 hover:opacity-100'}`}>
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400 mt-1 mb-2">
                        ...
                        <span>{format(startTime, "dd 'de' MMMM", { locale: ptBR })}</span>
                      </div>
                      {app.total_price > 0 && <span className="text-xs px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 font-bold border border-green-200 dark:border-green-800 rounded">Total: R$ {app.total_price}</span>}
                    </div>
                  </div>
                    ...
                    {isMine ? (
                      app.status === 'pending' ? (
                        <>
                          ...
                          <button onClick={() => updateStatus(app.id, 'cancelled')} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-slate-50 transition-colors"><X size={16} /></button>
                        </>
                      ) : (
                        <div className="flex items-center gap-2">
                          ...
                          {app.status === 'confirmed' && (
                            <>
                              <button onClick={() => updateStatus(app.id, 'completed')} className="ml-2 bg-green-50 dark:bg-green-900/30 border border-green-200 p-2 rounded-lg text-green-600 hover:bg-green-100 transition-colors" title="Finalizar Corte"><CheckCircle size={20} /></button>
                              <button onClick={() => { if(window.confirm('Cancelar agendamento?')) updateStatus(app.id, 'cancelled'); }} className="ml-1 bg-red-50 dark:bg-red-900/30 border border-red-200 p-2 rounded-lg text-red-600 hover:bg-red-100 transition-colors" title="Cancelar"><X size={20} /></button>
                              <button onClick={() => handleOpenNoShow(app)} disabled={!isTodayApp} className={`ml-1 p-2 rounded-lg border transition-colors ${isTodayApp ? 'bg-red-50 border-red-200 text-red-500 hover:bg-red-100' : 'opacity-30 cursor-not-allowed'}`} title="Não veio"><UserX size={20} /></button>
                            </>
                          )}
                        </div>
                      )
